...
Future<List<ListaCompras>> _fetchListas(String status) async {
  ...
  final listasRaw = await supabase
...
final listasAtivasProvider = FutureProvider<List<ListaCompras>>((ref) => _fetchListas('Ativa'));
final listasConcluidasProvider = FutureProvider<List<ListaCompras>>((ref) => _fetchListas('Concluida'));

/// Itens de uma lista especifica, ja separados por marcado/nao marcado.
final listaItensProvider = FutureProvider.family<List<Map<String, dynamic>>, String>((ref, listaId) async {
  final supabase = SupabaseHelper.instance.client;
  final raw = await supabase
      .from(SupabaseHelper.tableListaCompras)
      .select('*, produtos_catalogo(nome, emoji, categoria_id, categorias_produtos(nome))')
      .eq('lista_id', listaId)
      .filter('deleted_at', 'is', null)
      .order('updated_at', ascending: false);
...
final precosAnterioresProvider = FutureProvider.family<Map<String, double>, ({List<String> produtoIds, String excetoListaId})>((ref, params) async {
...
class ListasComprasRepo {
  ...
  static Future<String> criar(String nome) async {
    final res = await _client.from(SupabaseHelper.tableListasCompras).insert({'nome': nome.trim()}).select('id').single();
    return res['id'].toString();
  }

  static Future<void> renomear(String id, String novoNome) async {
    await _client.from(SupabaseHelper.tableListasCompras).update({
      'nome': novoNome.trim(),
  ...
  static Future<void> excluir(String id) async {
    ...
    await _client.from(SupabaseHelper.tableListaCompras).update({'deleted_at': now}).eq('lista_id', id);
    await _client.from(SupabaseHelper.tableListasCompras).update({'deleted_at': now}).eq('id', id);
  }
  ...
  static Future<String> copiarParaNovaLista(String listaOrigemId, String novoNome) => duplicar(listaOrigemId, novoNome);
