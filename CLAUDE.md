...
First-run spotlight walkthrough, driven by [lib/providers/tutorial_provider.dart](lib/providers/tutorial_provider.dart): `TutorialController` (a `Notifier<TutorialState>`) steps through `tutorialSteps` (target `GlobalKey` + copy), rendered by [lib/widgets/tutorial_overlay.dart](lib/widgets/tutorial_overlay.dart) via `tutorialNavigatorKey`. Target widgets across the app register themselves with the fixed `GlobalKey`s in [lib/utils/tutorial_keys.dart](lib/utils/tutorial_keys.dart) (e.g. `TutorialKeys.dashboardFab`, `TutorialKeys.shoppingAddFab`). Completion/pending-start flags persist via `shared_preferences` (`tutorial_completed`, `tutorial_pending_start`). Triggered once from `MainScreen._checkOnboarding`, right after `OnboardingScreen` (first-run account setup) or on next launch if `consumeTutorialPendingStart()` returns true. **Note:** `currentTabProvider` (drives `MainScreen`'s bottom-nav index) also lives in `tutorial_provider.dart`, not a separate navigation file — easy to miss when searching for it.
